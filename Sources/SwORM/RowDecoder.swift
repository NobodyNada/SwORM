...
///A decoder that can convert a `Row` into a Swift object.
...
private class KeyedDecoder<K: CodingKey>: KeyedDecodingContainerProtocol {
    var codingPath: [CodingKey]
    var allKeys: [K] {
        return decoder.row.columnIndices.keys.compactMap { K(stringValue: $0) }
    }
    ...
    func decodeNil(forKey key: K) throws -> Bool {
        guard let columnIndex = decoder.row.columnIndices[key.stringValue] else {
            throw DecodingError.keyNotFound(key, .init(
                codingPath: codingPath,
                debugDescription: "Key \(key) not found (valid keys are \(decoder.row.columnIndices.keys)"
                )
            )
        }
        if case .null = decoder.row.columns[columnIndex] { return true }
        else { return false }
    }
    
    func decode<T: DatabaseType & Decodable>(_ type: T.Type, forKey key: K) throws -> T {
        guard let result = try decoder.row.column(named: key.stringValue) as T? else {
...
private class SingleValueDecoder: SingleValueDecodingContainer {
    ...
    func decodeNil() -> Bool {
        if let index = decoder.row.columnIndices[key.stringValue] {
            if case .null = decoder.row.columns[index] { return true }
            else { return false }
        }
        return true
    }
